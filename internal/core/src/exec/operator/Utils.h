...
PrepareVectorIteratorsFromIndex(const SearchInfo& search_info,
    ...
    // when we use post filtered search, we will use vector iterator to continously get results and check scalar attr on them
    // until we get valid topk results
    if (search_info.group_by_field_id_.has_value() ||
        search_info.filter_execution_algo == "post") {
        try {
