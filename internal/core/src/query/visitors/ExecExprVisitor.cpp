...
ExecExprVisitor::ExecUnaryRangeVisitorDispatcher(UnaryRangeExpr& expr_raw) -> BitsetType {
    ...
    switch (op) {
        ...
        case OpType::NotEqual: {
            auto index_func = [&](Index* index) { return index->NotIn(1, &val); };
            auto elem_func = [&](T x) { return (x != val); };
            return ExecRangeVisitorImpl<T>(field_id, index_func, elem_func);
        }
        case OpType::GreaterEqual: {
            auto index_func = [&](Index* index) { return index->Range(val, OpType::GreaterEqual); };
            auto elem_func = [&](T x) { return (x >= val); };
            return ExecRangeVisitorImpl<T>(field_id, index_func, elem_func);
        }
        case OpType::GreaterThan: {
            auto index_func = [&](Index* index) { return index->Range(val, OpType::GreaterThan); };
            auto elem_func = [&](T x) { return (x > val); };
            return ExecRangeVisitorImpl<T>(field_id, index_func, elem_func);
        }
        case OpType::LessEqual: {
            auto index_func = [&](Index* index) { return index->Range(val, OpType::LessEqual); };
            auto elem_func = [&](T x) { return (x <= val); };
            return ExecRangeVisitorImpl<T>(field_id, index_func, elem_func);
        }
        case OpType::LessThan: {
            auto index_func = [&](Index* index) { return index->Range(val, OpType::LessThan); };
            auto elem_func = [&](T x) { return (x < val); };
            return ExecRangeVisitorImpl<T>(field_id, index_func, elem_func);
        }
...
auto
ExecExprVisitor::ExecUnaryRangeVisitorDispatcherJson(UnaryRangeExpr& expr_raw) -> BitsetType {
...
#define UnaryRangeJSONCompareNotEqual(cmp)                    \
    ...
    switch (op) {
        case OpType::Equal: {
            auto elem_func = [&](const milvus::Json& json) { UnaryRangeJSONCompare(x.value() == val); };
            return ExecRangeVisitorImpl<milvus::Json>(field_id, index_func, elem_func);
        }
        case OpType::NotEqual: {
            auto elem_func = [&](const milvus::Json& json) { UnaryRangeJSONCompareNotEqual(x.value() != val); };
            return ExecRangeVisitorImpl<milvus::Json>(field_id, index_func, elem_func);
        }
        case OpType::GreaterEqual: {
            auto elem_func = [&](const milvus::Json& json) { UnaryRangeJSONCompare(x.value() >= val); };
        ...
        case OpType::Equal: {
            switch (arith_op) {
                case ArithOpType::Add: {
                    ...
                    auto elem_func = [&](const milvus::Json& json) {
                        BinaryArithRangeJSONCompare(x.value() + right_operand == val);
                    };
                ...
                case ArithOpType::Mod: {
                    auto index_func = [val, right_operand](Index* index, size_t offset) { return false; };
                    auto elem_func = [&](const milvus::Json& json) {
                        BinaryArithRangeJSONCompare(static_cast<ExprValueType>(fmod(x.value(), right_operand)) == val);
                    };
                    return ExecDataRangeVisitorImpl<milvus::Json>(expr.column_.field_id, index_func, elem_func);
                }
                default: {
        ...
        case OpType::NotEqual: {
            switch (arith_op) {
                case ArithOpType::Add: {
                    auto index_func = [val, right_operand](Index* index, size_t offset) { return false; };
                    auto elem_func = [&](const milvus::Json& json) {
                        BinaryArithRangeJSONCompareNotEqual(x.value() + right_operand != val);
                    };
                    return ExecDataRangeVisitorImpl<milvus::Json>(expr.column_.field_id, index_func, elem_func);
                }
                case ArithOpType::Sub: {
                ...
                case ArithOpType::Mod: {
                    ...
                    auto elem_func = [&](const milvus::Json& json) {
                        BinaryArithRangeJSONCompareNotEqual(
                            static_cast<ExprValueType>(fmod(x.value(), right_operand)) != val);
                    };
                    return ExecDataRangeVisitorImpl<milvus::Json>(expr.column_.field_id, index_func, elem_func);
                }
                default: {
...
#define BinaryRangeJSONCompare(cmp)                           \
    ...
    if (lower_inclusive && upper_inclusive) {
        auto elem_func = [&](const milvus::Json& json) { BinaryRangeJSONCompare(val1 <= value && value <= val2); };
        ...
        case DataType::JSON: {
            switch (expr.val_case_) {
                ...
                case proto::plan::GenericValue::ValCase::kStringVal:
                    res = ExecUnaryRangeVisitorDispatcherJson<std::string>(expr);
                    break;
                default:
                    PanicInfo("unknown data type: " + std::to_string(expr.val_case_));
            }
            break;
        }
        default:
            PanicInfo("unsupported data type: " + std::to_string((int)expr.column_.data_type));
    }
    AssertInfo(res.size() == row_count_, "[ExecExprVisitor]Size of results not equal row count");
    bitset_opt_ = std::move(res);
}
    ...
    switch (expr.column_.data_type) {
        ...
        case DataType::JSON: {
            switch (expr.val_case_) {
                ...
                default: {
                    PanicInfo("unsupported value type {} in expression" + std::to_string(expr.val_case_));
                }
            }
            break;
        }
    ...
    for (int64_t chunk_id = 0; chunk_id < num_chunk; ++chunk_id) {
        ...
        auto getChunkData = [&, chunk_id](DataType type, FieldId field_id,
            ...
            switch (type) {
                ...
                default:
                    PanicInfo("unsupported data type: " + std::to_string((int)type));
            }
        };
...
template <typename ExprValueType>
auto
ExecExprVisitor::ExecTermVisitorImplTemplateJson(TermExpr& expr_raw) -> BitsetType {
    using Index = index::ScalarIndex<milvus::Json>;
...
ExecExprVisitor::visit(ExistsExpr& expr) {
    ...
    switch (expr.column_.data_type) {
        case DataType::JSON: {
            ...
            res = ExecRangeVisitorImpl<milvus::Json>(expr.column_.field_id, index_func, elem_func);
            break;
        }
        default:
            PanicInfo("unsupported data type: " + std::to_string((int)expr.column_.data_type));
