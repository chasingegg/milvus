            ...
            if (filter_only) {
                ...
                auto internal_segment =
                    dynamic_cast<milvus::segcore::SegmentInternalInterface*>(
                        segment);
                auto active_count =
                    internal_segment->get_active_count(timestamp);

                result = new milvus::SearchResult();
                result->total_nq_ = 0;
                result->unity_topK_ = 0;
                result->total_data_cnt_ = 0;
                result->segment_ = segment;

                if (active_count == 0) {
                    result->valid_count_ = 0;
                } else {
                    // Execute filter using the plan's filter nodes
                    auto plan_fragment = milvus::plan::PlanFragment(
                    ...
                    auto row = milvus::query::ExecPlanNodeVisitor::ExecuteTask(
                        plan_fragment, query_context);

                    // Extract bitset from result
                    auto col_vec =
                        std::dynamic_pointer_cast<milvus::ColumnVector>(
                            row->childrens()[0]);
                    milvus::BitsetTypeView view(col_vec->GetRawData(),
                                                col_vec->size());

                    // Calculate valid count: bitset has 1=filtered out, 0=pass
                    result->valid_count_ = active_count - view.count();
                }
            } else {
                // Normal search mode
