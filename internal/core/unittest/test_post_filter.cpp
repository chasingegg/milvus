...
/**
 * this UT is to cover post filtering execution logic (knowhere iterator next() -> scalar filtering)
...
void
CheckFilterSearchResult(const SearchResult& search_result_by_post_filter,
    ...
    ASSERT_EQ(search_result_by_post_filter.seg_offsets_.size(), topK * nq);
    ASSERT_EQ(search_result_by_post_filter.distances_.size(), topK * nq);

    for (int i = 0; i < topK * nq; ++i) {
        std::cout << search_result_by_pre_filter.seg_offsets_[i] << " "
                  << search_result_by_pre_filter.distances_[i] << " "
                  << search_result_by_post_filter.seg_offsets_[i] << " "
                  << search_result_by_post_filter.distances_[i] << std::endl;
...
TEST(PostFilter, GrowingRawData) {
    int dim = 128;
    uint64_t seed = 512;
    auto schema = std::make_shared<Schema>();
