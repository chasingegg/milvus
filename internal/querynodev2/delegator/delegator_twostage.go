...
func (sd *shardDelegator) twoStageSearch(
	...
	log.Info("Starting filter stage")
	req.FilterOnly = true
	filterResults, err := sd.executeSearchSubTasks(ctx, req, sealed, growing, sealedRowCount)
	if err != nil {
	...
	for _, result := range filterResults {
		for _, validCount := range result.GetFilterValidCounts() {
			if validCount > 0 {
				log.Info("valid count", zap.Int64("validCount", validCount), zap.Int("totalSegments", totalSegments))
				totalSegments++
			}
		}
	}

	log.Info("Two-stage search: filter stage completed",
		zap.Int("totalSegments", totalSegments),
	)

	// ==================== Optimize with actual stats ====================
	log.Info("Optimizing search params with actual stats")
	req.FilterOnly = false
	optimizedReq, err := optimizers.OptimizeSearchParams(ctx, req, sd.queryHook, finalSealedNum)
	if err != nil {
	...
	log.Info("Starting vector search stage with optimized params")
